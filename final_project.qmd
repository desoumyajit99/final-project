...
#density plot of poverty by state
pov_bp <- pvt_d %>%
...
pov_dp <- pvt_d %>%
          ...
          geom_density(aes(fill = after_stat(density)), alpha = 0.6) +
...
econ_edu_hist <- pvt_d %>%
          pivot_longer(all_of(econ_educ_vars), names_to = "Variable", values_to = "Value") %>%
                                  ggplot(aes(x = Value)) +
                                  geom_density(aes(fill = after_stat(density))) +
                                  facet_wrap(~ Variable, scales = "free") +
                                  labs(title = "Distributions: Economic & Education Variables", x = "Percentage", y = "Count") +
                                  theme_bw()
...
ed_pvt_sc <- pvt_d %>%
  ...
  geom_point(alpha = 0.3, aes(color = EduPercent)) +
  geom_smooth(method = "loess", se = FALSE, color = "black") +
...
agmc_pvt_sc <- pvt_d %>%
  pivot_longer(all_of(age_commute_vars),
               names_to = "Variable",
               values_to = "Value") %>%
  ggplot(aes(x = Value, y = Poverty)) +
  geom_point(alpha = 0.3, aes(color = Value)) +
  geom_smooth(method = "loess", se = FALSE, color = "black") +
  scale_color_continuous_sequential() +
  facet_wrap(~ Variable, scales = "free_x") +
  labs(title = "Poverty vs AvgAge and MeanCommute", x = NULL, y = "Poverty (%)") +
  theme_bw()
...
hg_pw <- pvt_d %>% 
                  ...
                  labs(title = "Distribution of Proportion Women", x = "Women (%)", y = "Count") +
                  theme_bw()

# poverty vs proportion women
wm_pvt_sc <- pvt_d %>% 
                        ggplot (aes(x = propWomen, y = Poverty)) +
                        geom_point(alpha = 0.3, aes(color = propWomen)) +
                        geom_smooth(method = "loess", se = FALSE, color = "black") +
                        scale_color_continuous_sequential() +
...
re_hist <- pvt_d %>%
  ...
  labs(title = "Distributions: Race/Ethnicity Percentages",  x = "Percent", y = "Count") +
  theme_bw()

# poverty vs race 
pr_pvt_sc <- pvt_d %>%
  pivot_longer(all_of(race_vars),
               names_to = "Race",
               values_to = "Percent") %>%
  ggplot(aes(x = Percent, y = Poverty)) +
  geom_point(alpha = 0.3, aes(color = Percent)) +
  geom_smooth(method = "loess", se = FALSE, color = "black") +
  scale_color_continuous_sequential() +
...
eps_hist <- pvt_d %>%
  ...
  labs(title = "Distributions: Employment Sectors", x = "Percent in sector (%)", y = "Count") + 
  theme_bw()

#poverty vs employment
pe_pvt_sc pvt_d %>%
  pivot_longer(all_of(sector_vars),
               names_to = "Sector",
               values_to = "Percent") %>%
  ggplot(aes(x = Percent, y = Poverty)) +
  geom_point(alpha = 0.3, aes(color = Percent)) +
...
ts_hist <- pvt_d %>%
  pivot_longer(all_of(transport_vars),
               names_to = "Mode",
               values_to = "Percent") %>%
  ggplot(aes(x = Percent)) +
  geom_histogram(bins = 30, aes(fill = after_stat(count))) +
  facet_wrap(~ Mode, scales = "free") +
  labs(title = "Distributions: Commuting Modes", x = "Percent using mode (%)", y = "Count") +
  theme_bw()
...
wt_hist <- pvt_d %>%
  pivot_longer(all_of(worktype_vars),
               names_to = "WorkType",
               values_to = "Percent") %>%
  ggplot(aes(x = Percent)) +
  geom_histogram(bins = 30, aes(fill = after_stat(count))) +
  facet_wrap(~ WorkType, scales = "free") +
  labs(title = "Distributions: Work Types", x = "Percent in work type (%)", y = "Count") +
  theme_bw()
...
pop_hist <- pvt_d %>%
                      ...
                      geom_histogram(bins = 30, aes(fill = after_stat(count))) + 
                      labs(title = "Distribution of County Population Size", x = "Total population", y = "Count") +
                      theme_bw() 

# poverty vs totalpop 
pop_pvt_ss <- pvt_d %>% 
                        ggplot(aes(x = TotalPop, y = Poverty)) +
                        geom_point(alpha = 0.8, aes(color = TotalPop)) +
                        labs(title = "Poverty vs Population Size", x = "Total population (log scale)", y = "Poverty (%)") +
                        theme_bw()
